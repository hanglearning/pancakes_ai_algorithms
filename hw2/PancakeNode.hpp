...
class PancakeNode {
    ...
    friend class PancakeGraph;  // so PancakeGraph class can access the private fields of this node
...
public:
    ...
    int getLargestPancakeIdOutOfPlace();  //function used to get the largest pancake id that's still out of place to set the heuristic cost of a certain pancake node
